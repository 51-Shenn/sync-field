...
export function Avatar({ name, size = "md", className }: { name: string; size?: "sm" | "md" | "lg" | "xl"; className?: string }) { const index = name.charCodeAt(0) % avatarColors.length; return <AvatarPrimitive.Root title={name} className={cn("inline-flex shrink-0 items-center justify-center rounded-full font-semibold ring-2 ring-white", avatarColors[index], size === "sm" && "size-7 text-[10px]", size === "md" && "size-9 text-xs", size === "lg" && "size-11 text-sm", size === "xl" && "size-16 text-lg", className)}><AvatarPrimitive.Fallback>{initials(name)}</AvatarPrimitive.Fallback></AvatarPrimitive.Root>; }
