...
const capabilities = [
  { icon: LayoutDashboard, title: "One operational view", description: "See project health, schedules, budgets, crews, and open issues without chasing updates." },
  { icon: ClipboardCheck, title: "Field reporting", description: "Capture daily progress and site issues in a consistent workflow built for the field." },
  { icon: Users, title: "Workforce coordination", description: "Know who is available, where crews are assigned, and where capacity is getting tight." },
  { icon: Fuel, title: "Resource control", description: "Track equipment, materials, fuel, and budget allocation across your entire portfolio." },
];
...
function LandingAction({ compact = false }: { compact?: boolean }) {
  ...
  if (session?.user) return <Link href="/control-centre" className={`${compact ? "h-10 px-4 text-sm" : "h-12 px-6"} inline-flex items-center justify-center gap-2 rounded-xl bg-orange-500 font-semibold text-white shadow-lg shadow-orange-500/20 transition hover:bg-orange-600`}>Open workspace <ArrowRight className="size-4" /></Link>;
  ...
  >
    {signingIn ? <LoaderCircle className="size-4 animate-spin" /> : null}{signingIn ? "Connecting to Google..." : "Open workspace"}{!signingIn && <ArrowRight className="size-4" />}
  </button>;
}
      ...
      <div className="grid min-h-[430px] md:grid-cols-[190px_1fr]">
        <aside className="hidden border-r border-white/10 bg-[#132238] p-4 md:block">
          ...
          <div className="mt-9 space-y-2 text-xs text-slate-400">
            {[[LayoutDashboard,"Control Centre"],[ClipboardCheck,"Site Reporting"],[HardHat,"Projects"],[Users,"Workforce"],[Fuel,"Resources"]].map(([Icon,label], index) => { const IconComponent = Icon as typeof LayoutDashboard; return <div key={String(label)} className={`flex items-center gap-2.5 rounded-lg px-3 py-2.5 ${index === 0 ? "bg-orange-500 text-white" : ""}`}><IconComponent className="size-4" />{String(label)}</div>; })}
          </div>
        </aside>
        <div className="bg-slate-50">
          ...
          <div className="p-5 sm:p-7">
            ...
            <div className="mt-4 grid gap-3 lg:grid-cols-[1.4fr_1fr]">
              <div className="rounded-xl border border-slate-200 bg-white p-4"><div className="flex items-center justify-between"><p className="text-xs font-semibold text-slate-900">Project progress</p><BarChart3 className="size-4 text-slate-400" /></div><div className="mt-5 space-y-4">{[["Harbour Residences",78],["Northbank Offices",61],["Civic Arts Centre",44]].map(([name,value]) => <div key={String(name)}><div className="mb-1.5 flex justify-between text-[10px] text-slate-500"><span>{name}</span><span>{value}%</span></div><div className="h-1.5 rounded-full bg-slate-100"><div className="h-full rounded-full bg-orange-500" style={{width:`${value}%`}} /></div></div>)}</div></div>
              <div className="rounded-xl border border-slate-200 bg-white p-4"><p className="text-xs font-semibold text-slate-900">Today on site</p><div className="mt-4 space-y-3">{["Concrete pour completed","Safety walk signed off","RFI #184 submitted"].map(item => <div key={item} className="flex gap-2 text-[10px] text-slate-500"><CheckCircle2 className="size-3.5 shrink-0 text-emerald-500" />{item}</div>)}</div></div>
            </div>
          </div>
        </div>
      </div>
  ...
  return <div className="min-h-screen overflow-hidden bg-[#f8fafc] text-slate-950">
    <header className="fixed inset-x-0 top-0 z-50 border-b border-slate-200/70 bg-white/85 backdrop-blur-xl">
      <nav className="mx-auto flex h-16 max-w-7xl items-center px-5 lg:px-8">
        ...
        <button className="ml-auto rounded-lg p-2 text-slate-700 md:hidden" onClick={() => setMobileOpen(!mobileOpen)} aria-label="Toggle navigation">{mobileOpen ? <X /> : <Menu />}</button>
      </nav>
    ...
    <main>
      <section className="relative bg-[#132238] px-5 pb-24 pt-36 text-white lg:px-8 lg:pb-32 lg:pt-44">
        ...
        <div className="relative mx-auto max-w-7xl text-center">
          <div className="mx-auto inline-flex items-center gap-2 rounded-full border border-orange-400/25 bg-orange-400/10 px-3 py-1.5 text-xs font-semibold text-orange-300"><HardHat className="size-3.5" />Built for teams that build</div>
