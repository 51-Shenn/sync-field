...
export function ProjectCard({ project, teamMembers = [] }: { project: Project; teamMembers?: { id: string; name: string }[] }) {
...
export function ProjectLegend({ projects }: { projects: Project[] }) { return <div className="flex flex-wrap gap-4">{projects.slice(0,4).map(p=><span key={p.id} className="flex items-center gap-1.5 text-[11px] text-slate-500"><i className="size-2 rounded-full" style={{backgroundColor:p.color}}/>{p.name}</span>)}</div>; }
