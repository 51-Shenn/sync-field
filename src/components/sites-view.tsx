...
import { FileImage, FileText, Paperclip, Plus, Search, Trash2 } from "lucide-react";
...
export function SitesView() {
  ...
  return <>
    <div className="mb-5 flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-3 sm:flex-row">
      <div className="relative flex-1"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-400" /><Input className="pl-9" placeholder="Search reports..." value={query} onChange={e => setQuery(e.target.value)} /></div>
      ...
      <Dialog trigger={<Button><Plus className="size-4" />New report</Button>} title="Create site report" description="Log an update or issue with optional attachments.">
        <form className="space-y-4" onSubmit={e => { e.preventDefault(); addReport(); }}>
          ...
          <div><Label>Attachments</Label><div className="flex gap-2"><Input value={attachInput} onChange={e => setAttachInput(e.target.value)} placeholder="Filename or URL" /><Button type="button" variant="outline" onClick={addAttachment}><Paperclip className="size-4" /></Button></div>{form.attachments.length > 0 && <div className="mt-2 space-y-1">{form.attachments.map((a, i) => <div key={i} className="flex items-center gap-2 rounded-md bg-slate-50 px-3 py-1.5 text-xs text-slate-600"><FileText className="size-3.5" />{a}</div>)}</div>}</div>
          <Button type="submit" className="w-full">Create report</Button>
        </form>
      </Dialog>
    </div>
    <div className="space-y-4">{filtered.map(report => { const project = projects.find(p => p.id === report.projectId); const author = teamMembers.find(m => m.id === report.createdBy); return <Card key={report.id} className="transition-shadow hover:shadow-md"><CardContent><div className="flex items-start justify-between gap-4"><div className="flex-1"><div className="flex items-center gap-2"><Badge>{report.type}</Badge><Badge value={report.status} /><span className="text-xs text-slate-400">{report.createdAt}</span></div><h3 className="mt-3 font-semibold text-slate-950">{report.title}</h3><p className="mt-2 text-sm leading-6 text-slate-600">{report.description}</p><div className="mt-4 flex flex-wrap items-center gap-4 text-xs text-slate-500">{project && <span className="flex items-center gap-1.5"><span className="size-2 rounded-full" style={{ backgroundColor: project.color }} />{project.name}</span>}{author && <span className="flex items-center gap-1.5"><Avatar name={author.name} size="sm" />{author.name}</span>}</div>{report.attachments.length > 0 && <div className="mt-4 flex flex-wrap gap-2">{report.attachments.map((att, i) => <span key={i} className="inline-flex items-center gap-1.5 rounded-md bg-slate-50 px-2.5 py-1 text-xs text-slate-600"><FileImage className="size-3.5" />{att}</span>)}</div>}</div><div className="flex flex-col gap-1"><Button variant={report.status === "open" ? "secondary" : "outline"} size="sm" onClick={() => toggleStatus(report.id)}>{report.status === "open" ? "Resolve" : "Reopen"}</Button><Button variant="ghost" size="icon" className="size-8 text-red-500 hover:bg-red-50" onClick={() => deleteReport(report.id)}><Trash2 className="size-3.5" /></Button></div></div></CardContent></Card>})}</div>
